...
Y1S: .install_feeds
	@cd $(openwrt_dir); \
		...
		cp -vf ../config-lenovo-y1s .config; \
...
.patched: .checkout_svn
	...
	@cp -vf config-lenovo-y1s $(openwrt_dir)/.config
	@touch .patched
...
menuconfig: .install_feeds
	...
	@cp -vf config-lenovo-y1s $(openwrt_dir)/.config
	@touch config-lenovo-y1s  # change modification time
	@make -C $(openwrt_dir) menuconfig
	@[ $(openwrt_dir)/.config -nt config-lenovo-y1s ] && cp -vf $(openwrt_dir)/.config config-lenovo-y1s || :
